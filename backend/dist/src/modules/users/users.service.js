...
let UsersService = class UsersService {
    ...
    async searchTenants(query) {
        return this.usersRepository
            ...
            .where('user.role = :role', { role: user_entity_1.UserRole.TENANT })
            .andWhere('(user.firstName ILIKE :query OR user.lastName ILIKE :query OR user.email ILIKE :query)', { query: `%${query}%` })
            .getMany();
    }
    async assignTenantToProperty(tenantId, propertyId, leaseStartDate, leaseEndDate) {
        const tenant = await this.findOne(tenantId);
        if (tenant.role !== user_entity_1.UserRole.TENANT) {
            throw new Error('User is not a tenant');
        }
        tenant.property = { id: propertyId };
        tenant.leaseStartDate = leaseStartDate;
        tenant.leaseEndDate = leaseEndDate;
        return this.usersRepository.save(tenant);
    }
};
