...
export default function AdminPaymentsPage() {
  ...
  const fetchTenants = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/users?role=tenant', {
        headers: {
  ...
  const handleRecordPayment = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/payments/create/${newPayment.tenantId}`, {
        method: 'POST',
        headers: {
                    ...
                    >
                      ...
                      {tenants.map((tenant) => (
                        <option key={tenant.id} value={tenant.id}>
                          {tenant.name || tenant.email} {tenant.property?.name ? `(${tenant.property.name})` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
